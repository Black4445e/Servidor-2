...
async def perform_lag(team_code: str):
    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            # SAI DO SQUAD IMEDIATAMENTE
            exit_packet = await ExiT(0, key, iv)
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', exit_packet)

            # Espera 0.15 segundos antes de repetir
            await asyncio.sleep(0.15)

        return {"status": "success", "message": "Entrou e saiu em loop por 45s!"}

    except Exception as e:
        raise Exception(f"Failed during join/exit loop: {str(e)}")
...
def join_team():
    ...
    if not team_code:
        return jsonify({"status": "erro", "mensagem": "Parâmetro 'tc' é obrigatório"})

    # Chama o perform_emote apenas com o team_code
    future = asyncio.run_coroutine_threadsafe(
        perform_lag(team_code), loop
    )
...
async def MaiiiinE():
    ...
    Uid , Pw = '4309458151','BY_MERCY-EDGMAAPJF-HACKED'
